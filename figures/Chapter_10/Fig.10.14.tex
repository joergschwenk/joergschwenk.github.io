...
\participants{\doesinabox{Client $C$\\ $[(sk_C,pk_C,cert_C)]$}}
...
\AfromB{SH,CRT,SKE,[CReq,]SHD}

\doesA{$[CRT \leftarrow (cert_C)]$}
\doesA{$x \rand \mathbb{Z}_q, X \leftarrow g^x \bmod p$}
\doesA{$CKE \leftarrow (X)$}
...
\subprotocolcaption{$pms \leftarrow CDH(Y,X)$}
\subprotocolcaption{$ms \leftarrow \PRF{pms}{l_1,r_C|r_S}$}
\subprotocolcaption{$k_{mac}^{CS}|k_{mac}^{SC}|k_{enc}^{CS}|k_{enc}^{SC}|IV^{CS}|IV^{SC} \leftarrow \PRF{ms}{l_2,r_S|r_C}$}
